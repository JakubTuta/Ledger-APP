...
async def compute_top_errors() -> None:
    ...
    logger.info("Starting top errors computation job")
    ...
    try:
        async with database.get_logs_session() as session:
            ...
            for project_id, errors in by_project.items():
                ...
                logger.debug(
                    f"Cached top errors for project {project_id}: {len(top_50)} errors"
                )

            logger.info(
                f"Top errors computation completed for {len(by_project)} projects"
            )
